 ...
 * @author HorridoJoho
 */
public abstract class YANModScript extends L2AttackableAIScript
{
	...
	{
		super(-1, name, SCRIPT_COLLECTION);
	...
	protected final boolean isInsideAnyZoneOf(L2Character character, byte first, byte... more)
	{
	...
	protected final void fillItemAmountMap(Map<Integer, Long> items, YANModProduct product)
	{
		...
		{
			Long amount = items.get(item.getValue().getItem().getItemId());
			if (amount == null)
			{
				amount = 0L;
			}
			items.put(item.getValue().getItem().getItemId(), amount + item.getValue().itemAmount);
		}
	}
	...
	{
		if (isInsideAnyZoneOf(player, L2Character.ZONE_PVP, L2Character.ZONE_SIEGE, L2Character.ZONE_WATER, L2Character.ZONE_JAIL, L2Character.ZONE_DANGERAREA))
		{
